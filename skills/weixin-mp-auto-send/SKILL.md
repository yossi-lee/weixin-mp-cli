...
**自动 Markdown 排版：** 如果用户提供的正文内容没有使用 Markdown 格式（例如纯文本、没有标题标记、段落没有空行分隔等），你需要自动识别并转换为 Markdown 格式：
- 识别标题：将明显的标题行（如短句、居中的文字、带有编号的段落等）转换为 `#`、`##`、`###` 等标题标记
- 识别段落：将连续的文字块用空行分隔为独立段落
- 识别列表：将带有编号或项目符号的行转换为有序/无序列表
- 保持原文内容不变：只做格式标记的添加，不修改、删除、重写任何文字内容
- 转换后告知用户已自动排版，并展示转换后的 Markdown 内容供确认
